...
### Brand & Accent
- **Teal** (`--color-primary` — `#1E97B2`): The brand's signature colour, matching the Umang website. **Fill-only.** `#1E97B2` on white text measures **3.43:1** and fails AA, so it never carries *body text* — it fills buttons, active states, and focus rings, always paired with navy `--color-on-primary` (`#0D2032`, **4.82:1** on teal). It does still colour a handful of small icons (e.g. `doctor/dashboard`'s inline `FileText`/`HeartPulse` glyphs) — WCAG 1.4.11 asks only 3:1 of graphical objects, and `#1E97B2` clears that (3.43:1), so an icon is not held to the 4.5:1 text floor the way a label or heading is.
- **Teal Dark** (`--color-primary-dark` — `#196b7e`): The text-safe teal. **6.09:1** with white text, so any teal fill that *carries white text* stays at this shade or darker — never as light as `#1E97B2`. In practice most such fills (avatar-initial badges, hero gradients, `--gradient-primary`) go a step further still, to `--color-primary-800`/`-900`, so a gradient's lightest stop clears 4.5:1 with headroom to spare rather than sitting right at the floor.
- **Teal Light** (`--color-primary-light` — `#6acdd9`): Light-teal tint for soft backgrounds and decorative accents; not a text colour, and never the light end of a gradient carrying white text (1.85:1 — worse than `#1E97B2` itself).
...
### Enforcement
`src/app/__tests__/theme-contrast.test.ts` doesn't re-assert the exact figures quoted above — it asserts the *thresholds* they need to clear: every token that carries text must measure `>= 4.5` against its paired ink, `--color-primary` against white must stay `< 4.5` (proving the fill/text split above stays justified), and `--color-brand-orange` against white must stay `< 3` (so brand orange can't silently become text-safe). It also scans every `.ts`/`.tsx` source line for `text-white` sharing a line with an unsafe teal fill — a bare `bg-primary`/`bg-primary-500`, or a `bg-`/`from-`/`via-`/`to-` naming `--color-primary`, `--color-primary-light`, or their literal hexes — and fails if any turn up. It asserts nothing about `--color-primary-light`'s own ratio; that token is guarded only through the fill-pairing scan just described. The retired blue and orange palettes (`#EE6B26`, `#B84A16`, and the old blue brand hexes) are blocked outright by a `no-restricted-syntax` ESLint rule in `eslint.config.mjs`, so they cannot silently reappear in a future edit.
