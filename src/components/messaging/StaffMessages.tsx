    ...
    <div className="grid lg:grid-cols-[330px_1fr] gap-4 flex-1 min-h-0">
      ...
      <div className="rounded-2xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06)] overflow-y-auto flex flex-col">
        <div className="p-3 border-b border-slate-100">
          <button onClick={() => setCompose(true)} className="w-full h-9 rounded-xl bg-[var(--color-primary-dark)] hover:bg-[#1a5667] text-white text-[13px] font-bold flex items-center justify-center gap-1.5"><Plus className="h-4 w-4" /> New message</button>
        </div>
        ...
        {mine.map(c => {
          ...
          return (
            <button key={c.id} onClick={() => setSelectedId(c.id)}
              className={cn("w-full text-left p-3.5 border-b border-slate-50 transition flex gap-3", selectedId === c.id ? "bg-[rgba(30,151,178,0.07)]/50" : "hover:bg-slate-50")}>
              <span className="h-10 w-10 rounded-full bg-gradient-to-br from-[var(--color-primary-900)] to-[var(--color-primary-800)] text-white flex items-center justify-center font-bold text-[13px] flex-shrink-0">{initials(other.name)}</span>
              <span className="flex-1 min-w-0">
                ...
                <span className="flex items-center justify-between gap-2">
                  ...
                  {unread > 0 && <span className="text-[10px] font-bold text-white bg-[var(--color-primary-dark)] rounded-full h-4.5 min-w-4.5 px-1 flex items-center justify-center flex-shrink-0">{unread}</span>}
                </span>
          ...
          return (
            <>
              <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-3">
                <span className="h-10 w-10 rounded-full bg-gradient-to-br from-[var(--color-primary-900)] to-[var(--color-primary-800)] text-white flex items-center justify-center font-bold text-[13px]">{initials(other.name)}</span>
                ...
                {selected.messages.map(m => {
                  ...
                  return (
                    <div key={m.id} className={cn("flex", mineMsg ? "justify-end" : "justify-start")}>
                      <div className={cn("max-w-[78%] rounded-2xl px-3.5 py-2", mineMsg ? "bg-[var(--color-primary-dark)] text-white rounded-br-md" : "bg-white text-slate-800 border border-slate-100 rounded-bl-md")}>
              ...
              <div className="p-3 border-t border-slate-100 flex items-center gap-2">
                <input value={reply} onChange={e => setReply(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') send() }}
                  placeholder={`Message ${other.name.split(' ')[0]}…`} className="flex-1 h-10 px-3.5 rounded-xl bg-slate-50 border border-slate-200 text-[14px] text-slate-800 outline-none focus:border-[rgba(30,151,178,0.30)] focus:ring-2 focus:ring-primary/20" />
                <button onClick={send} disabled={!reply.trim()} aria-label="Send" className="h-10 w-10 rounded-xl bg-[var(--color-primary-dark)] disabled:bg-slate-200 text-white flex items-center justify-center active:scale-95 transition"><Send className="h-4 w-4" /></button>
              </div>
            </>
          )
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
        ...
        <div className="relative mb-3">
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search staff by name, role or department…"
            className="w-full h-10 pl-9 pr-3 rounded-xl bg-slate-50 border border-slate-200 text-[13.5px] text-slate-800 outline-none focus:border-[rgba(30,151,178,0.30)] focus:ring-2 focus:ring-primary/20" />
        </div>
        <div className="max-h-52 overflow-y-auto -mx-1 px-1 space-y-1 mb-3">
          {list.map(c => (
            <button key={c.id} onClick={() => setToId(c.id)}
              className={cn("w-full text-left flex items-center gap-3 p-2.5 rounded-xl transition", toId === c.id ? "bg-[rgba(30,151,178,0.07)] ring-1 ring-primary/25" : "hover:bg-slate-50")}>
              <span className="h-9 w-9 rounded-full bg-gradient-to-br from-[var(--color-primary-900)] to-[var(--color-primary-800)] text-white flex items-center justify-center font-bold text-[12px]">{initials(c.name)}</span>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={submit} className="flex-1 h-11 rounded-xl bg-[var(--color-primary-dark)] hover:bg-[#1a5667] text-white font-bold text-[13.5px] flex items-center justify-center gap-2"><Send className="h-4 w-4" /> Send</button>
        </div>
      </motion.div>
    </motion.div>
  )
