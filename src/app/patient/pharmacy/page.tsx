...
const RX_STATUS: Record<PrepStatus, { label: string; tint: string }> = {
  ...
  preparing: { label: "Being prepared", tint: "bg-[rgba(30,151,178,0.07)] text-[var(--color-accent)]" },
    ...
    <div className="max-w-4xl mx-auto pb-10 space-y-5">
              ...
              <div key={rx.id} className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {rx.medicines.map((m, i) => {
                    ...
                    return (
                      <span key={i} className={cn("text-[12px] font-semibold px-2 py-0.5 rounded-full inline-flex items-center gap-1", unavailable ? "bg-red-50 text-red-600" : substituted ? "bg-[rgba(30,151,178,0.07)] text-[var(--color-accent)]" : "text-slate-600 bg-slate-100")}>
                        {m.name} ×{m.quantity}
            ...
            <div key={m.name} className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  <span className="h-10 w-10 rounded-2xl bg-[rgba(30,151,178,0.07)] flex items-center justify-center flex-shrink-0"><Pill className="h-5 w-5 text-[var(--color-accent)]" /></span>
                  <div>
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isOrdered ? "bg-green-50 text-green-700 cursor-default" : "bg-[var(--color-primary-dark)] text-white",
                  )}
                >
      ...
      <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-[15px] font-bold text-slate-900 flex items-center gap-2">
            <span className="h-10 w-10 rounded-2xl bg-[rgba(30,151,178,0.07)] flex items-center justify-center flex-shrink-0"><Truck className="h-5 w-5 text-[var(--color-accent)]" /></span>
            Home delivery
          </h3>
          <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-[rgba(30,151,178,0.07)] text-[var(--color-accent)]">ETA today, 6:30 PM</span>
        </div>
          ...
          {DELIVERY_STAGES.map((s, i) => (
            <div key={s.label} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-1">
                <div className={cn("h-8 w-8 rounded-full flex items-center justify-center", i <= DELIVERY_CURRENT ? "bg-[var(--color-primary-dark)] text-white" : "bg-slate-100 text-slate-400")}>
      ...
      <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
        <h3 className="text-[15px] font-bold text-slate-900 flex items-center gap-2 mb-3">
          <span className="h-10 w-10 rounded-2xl bg-[rgba(30,151,178,0.07)] flex items-center justify-center flex-shrink-0"><Receipt className="h-5 w-5 text-[var(--color-accent)]" /></span>
          Pharmacy bills
        </h3>
