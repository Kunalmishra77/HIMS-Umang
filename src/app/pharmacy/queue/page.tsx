...
const SOURCE_STYLE: Record<RxSource, string> = {
  OPD: "bg-[rgba(30,151,178,0.07)] text-[var(--color-accent)] ring-primary/25",
  IPD: "bg-[rgba(30,151,178,0.07)] text-[var(--color-accent)] ring-primary/25",
  ICU: "bg-red-50 text-red-700 ring-red-200",
  OT: "bg-[rgba(30,151,178,0.07)] text-[var(--color-accent)] ring-primary/25",
  "Home Rx": "bg-[rgba(30,151,178,0.07)] text-[var(--color-accent)] ring-primary/25",
  Discharge: "bg-amber-50 text-amber-700 ring-amber-200",
}
const STATUS_STYLE: Record<PrepStatus, string> = {
  queued: "bg-amber-100 text-amber-700",
  preparing: "bg-[rgba(30,151,178,0.12)] text-[var(--color-accent)]",
          ...
          <div className="flex-shrink-0 flex items-center gap-2 flex-wrap justify-end">
            ...
            {[t("queue.collectorSelf"), t("queue.collectorRelative"), t("queue.collectorNurse")].map(c => (
              <button key={c} onClick={() => props.setCollector(c)}
                className={cn("text-[11px] font-semibold px-2 py-1 rounded-lg cursor-pointer", props.collector === c ? "bg-[rgba(30,151,178,0.12)] text-[var(--color-accent)]" : "bg-slate-100 text-slate-600 hover:bg-slate-200")}>{c}</button>
            ))}
              ...
              return (
                <div key={i} className="rounded-lg bg-white ring-1 ring-slate-200/70 p-3">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-slate-800 flex items-center gap-1.5">
                        ...
                        {m.supply === "order_raised" && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-[rgba(30,151,178,0.12)] text-[var(--color-accent)]">{t("queue.poRaised")}</span>}
                        ...
                        {m.substitutedFrom && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-[rgba(30,151,178,0.12)] text-[var(--color-accent)] flex items-center gap-0.5"><Repeat className="h-2.5 w-2.5" />{t("queue.substitutedFrom", { name: m.substitutedFrom })}</span>}
                      </p>
                  ...
                  {oosLine && rx.status !== "collected" && (() => {
                    ...
                    if (picking) {
                      return (
                        <div className="mt-2 rounded-lg bg-[rgba(30,151,178,0.07)] ring-1 ring-primary/25 p-2">
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap">
                            {alts.map(a => (
                              <button key={a} onClick={() => props.onSubstitute(m.name, a)}
                                className="text-[11px] font-bold text-[var(--color-accent)] bg-white hover:bg-[rgba(30,151,178,0.14)] ring-1 ring-primary/25 px-2.5 py-1 rounded-lg cursor-pointer">{a}</button>
                            ))}
                    ...
                    return (
                      <div className="flex items-center gap-2 mt-2 flex-wrap">
                        {alts.length > 0 && (
                          <button onClick={() => props.onStartSubstitute(m)} className="flex items-center gap-1 text-[11px] font-bold text-[var(--color-accent)] bg-[rgba(30,151,178,0.07)] hover:bg-[rgba(30,151,178,0.14)] px-2.5 py-1 rounded-lg cursor-pointer"><Repeat className="h-3 w-3" />Substitute ({alts.length})</button>
                        )}
                        {m.supply !== "order_raised" && (
                          <button onClick={() => props.onOrder(m)} className="flex items-center gap-1 text-[11px] font-bold text-[var(--color-accent)] bg-[rgba(30,151,178,0.07)] hover:bg-[rgba(30,151,178,0.14)] px-2.5 py-1 rounded-lg cursor-pointer"><ShoppingCart className="h-3 w-3" />Order from inventory manager</button>
                        )}
...
function ActionBtn({ onClick, children, icon: Icon, tone }: { onClick: () => void; children: ReactNode; icon: ComponentType<{ className?: string }>; tone: "brand" | "ghost" }) {
  return (
    <button onClick={onClick}
      ...
      style={tone === "brand" ? { background: "linear-gradient(135deg,var(--color-primary-900),var(--color-primary-dark))", color: "#fff", boxShadow: "0 2px 8px rgba(30,151,178,0.25)" } : undefined}>
