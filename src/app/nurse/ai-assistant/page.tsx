    ...
    <div className="flex flex-col h-full min-h-0 gap-4">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2.5">
          <span className="h-9 w-9 rounded-xl bg-primary-dark flex items-center justify-center shadow-xs"><BrainCircuit className="h-4.5 w-4.5 text-white" /></span>
          <div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4 min-h-0 bg-surface-sunken">
          {messages.length === 0 && !thinking ? (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-lg mx-auto px-4">
              <span className="h-14 w-14 rounded-2xl bg-primary-dark flex items-center justify-center shadow-sm mb-4"><Sparkles className="h-7 w-7 text-white" /></span>
            ...
            <div className="max-w-3xl mx-auto space-y-4">
              {messages.map((m, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={cn("flex gap-2.5", m.role === "user" ? "justify-end" : "justify-start")}>
                  {m.role === "ai" && <span className="h-8 w-8 rounded-xl bg-primary-dark flex items-center justify-center flex-shrink-0 mt-0.5"><BrainCircuit className="h-4 w-4 text-white" /></span>}
              ...
              {thinking && (
                <div className="flex gap-2.5 justify-start">
                  <span className="h-8 w-8 rounded-xl bg-primary-dark flex items-center justify-center flex-shrink-0"><BrainCircuit className="h-4 w-4 text-white animate-pulse" /></span>
