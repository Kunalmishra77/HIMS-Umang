...
const bills = [
  ...
  {
    // Kiran Patil (PT-20394) is the demo patient account (see
    // scripts/seed/provision-demo-accounts.mjs). Without a bill here,
    // /patient/billing has nothing to show once that account is linked.
    id: 'BIL-opd-kiran', patient_id: 'PT-20394', payer_type: 'cash', status: 'open',
    lines: [{ id: 'L1', source: 'consult', name: 'OPD consultation', qty: 1, unitPrice: 500, total: 500, duplicateFlag: false }],
    total: 500, discount: 0, non_payable: 0, insurance_covered: 0, paid: 500, balance: 0,
  },
];

const PAID_IDS = ['BIL-opd-1', 'BIL-opd-kiran'];
...
for (const b of bills) {
  await c.query(`
    ...
    [b.id, b.patient_id, b.payer_type, b.payer_name ?? null, PAID_IDS.includes(b.id) ? 'paid' : b.status,
